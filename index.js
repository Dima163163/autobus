...
const getNextDeparture = (firstDepartureTime, frequencyMinutes) => {
  ...
  let departure = DateTime.now()
    .set({hour, minute, second: 0, milliseconds: 0})
    .setZone(timeZone)
...
[...buses].sort((a, b) => new Date(`${a.nextDeparture.date}T${a.nextDeparture.time}`) - new  Date(`${b.nextDeparture.date}T${b.nextDeparture.time}`));
...
wss.on('connection', (ws) => {
  ...
  const sendUpdates = async () => {
    try {
      ...
      console.err(`Error websoket connection: ${error}`);
    }
  };
